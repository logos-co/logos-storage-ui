...
// The node configuration form: reads config.json, edits it row by row and
// writes it back. Shared by the Settings page and the advanced onboarding step.
ScrollView {
    id: root
    ...
    // The onboarding has its own navigation, so it hides the row that would
    // restart the flow the user is already in.
    property bool showRestartOnboarding: true
    ...
    // Merge edited values onto the loaded config, preserving unknown keys. A
    // field left empty carries no key at all, so the node keeps its own default.
    function buildConfig() {
        ...
        put("listen-ip", root.vListenIp)
        putInt("listen-port", root.vListenPort)
        putInt("disc-port", root.vDiscPort)
        putInt("max-peers", root.vMaxPeers)
        put("storage-quota", root.vStorageQuota)
        put("mix-pool-json", root.vMixPoolJson)
        put("nat-schedule-interval", root.vNatSchedule)
        putInt("nat-port-mapping-discover-timeout", root.vNatDiscoverTimeout)
        putInt("nat-port-mapping-timeout", root.vNatMappingTimeout)
        putInt("nat-port-mapping-recheck-period", root.vNatRecheck)
    ...
    ColumnLayout {
        ...
        Section {
            text: "Advanced"
            visible: root.showRestartOnboarding
        }
