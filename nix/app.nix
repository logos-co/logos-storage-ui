...
pkgs.stdenv.mkDerivation rec {
  ...
  preFixup = ''
    ...
    find $out -type f -executable -exec sh -c '
      if file "$1" | grep -q "ELF.*executable"; then
        ...
        if echo "$1" | grep -q "/logos-storage-ui-app$"; then
  ...
  configurePhase = ''
    ...
    echo "Configuring logos-storage-ui-app..."
    ...
    echo "storage-ui: ${logosStorageUI}"
    ...
    test -d "${logosStorageUI}" || (echo "storage-ui not found" && exit 1)
  ...
  buildPhase = ''
    ...
    cmake --build build
    echo "logos-storage-ui-app built successfully!"
  ...
  installPhase = ''
    ...
    if [ -f "build/bin/logos-storage-ui-app" ]; then
      cp build/bin/logos-storage-ui-app "$out/bin/"
      echo "Installed logos-storage-ui-app binary"
    fi
...
cpp-sdk: ${logosSdk}
storage-module: ${logosStorageModule}
capability-module: ${logosCapabilityModule}
storage-ui: ${logosStorageUI}
